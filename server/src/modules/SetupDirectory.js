...
try {
    ...
    fs.exists(path.join(HOME_DIR, "render.sh"), exists => {
        if(!exists) {
            fs.exists(FOLDER_SCRIPTS, (exists => {
                if(exists) {
                    fs.readdir(FOLDER_SCRIPTS, (err, files) => {
                        ...
                        files.forEach(file => {
                            fs.rename(path.join(FOLDER_SCRIPTS,file), path.join(HOME_DIR, file))
                        })
                    })
                }else{
